...
def generate_signal_id(symbol: str, candle_time: str) -> str:
    """
    Deterministic unique ID: SYMBOL-YYYYMMDDHHMMSS. Same signal (same
    symbol + candle) always produces the same ID, useful for tracing
    a signal across Telegram, state, and paper trade records without
    needing a separate ID-issuing system.
    """
    clean_symbol = symbol.replace("/", "")
    clean_time = candle_time.replace("-", "").replace(":", "").replace(" ", "")
    return f"{clean_symbol}-{clean_time}"
...
def build_signal_message(signal: dict) -> str:
    return (
        ...
        f"RR: {signal['rr']:.2f}\n"
        f"Score: {signal['score_normalized']}/100 (raw {signal['score']}/{signal['max_possible_score']})\n"
        f"Conditions: {', '.join(signal['reasons'])}\n\n"
...
def setup_to_signal_dict(symbol: str, candle_time: str, setup) -> dict:
    return {
        "signal_id": generate_signal_id(symbol, candle_time),
        "symbol": symbol,
        ...
        "rr": setup.rr,
        "score": setup.score,
        "score_normalized": setup.score_normalized,
        "max_possible_score": setup.max_possible_score,
...
def check_stale(pending_entry: dict, current_closed_time: str,
                 h4, m15, comparison_m15, min_score: int) -> tuple:
    """
...
def process_pair(pair_config: dict, state: dict, pending: dict,
                  h4_cache: dict, open_trades: list, now_utc: datetime) -> dict:
    symbol = pair_config["symbol"]
    smt_symbol = pair_config["smt_symbol"]
    min_score = pair_config.get("min_score") or MIN_SCORE
    ...
    # --- Monitor any open paper trades for this symbol against fresh closed candles ---
    still_open = []
    for trade in open_trades:
        if trade["symbol"] != symbol:
            ...
            continue
        try:
            result = paper_trades.check_trade_against_candles(trade, m15)
        except Exception as e:
            print(f"[{symbol}] Error checking open trade: {e}")
            still_open.append(trade)
            continue

        if result is None:
            still_open.append(trade)  # still open, not resolved yet
        else:
            record = {**trade, **result}
            paper_trades.append_trade_history(record)
            health["trades_resolved"] += 1
            print(f"[{symbol}] Paper trade resolved: {result['outcome'].upper()} "
                  f"({result['outcome_r']:+.2f}R) at {result['exit_time']}")
    ...
    if symbol in pending:
        ...
        try:
            stale, reason = check_stale(entry, current_closed_time, h4, m15, comparison_m15, min_score)
        except Exception as e:
    ...
    if symbol not in pending:
        ...
        if setup is not None:
            ...
            if state.get(symbol) == current_closed_time:
                ...
                # Open the paper trade regardless of Telegram delivery -
                # tracking whether the SETUP wins is separate from
                # whether the notification successfully arrived.
                open_trades.append(paper_trades.open_trade(signal, current_closed_time))
...
def main():
    ...
    if is_market_closed(now_utc):
        print(f"Market closed (weekend) at {now_utc.strftime('%Y-%m-%d %H:%M:%S')} UTC - "
              f"skipping this run entirely, no API calls made.")
        append_health_record({
            "scan_time": now_utc.strftime("%Y-%m-%d %H:%M:%S"),
            "symbol": "ALL",
            "candle_time": None,
            "api_status": "market_closed",
            "setup_found": False,
            "telegram_status": "not_applicable",
            "trades_resolved": 0,
        })
    ...
    open_trades = paper_trades.load_open_trades()
    ...
    trades_before = json.dumps(open_trades, sort_keys=True)

    for pair_config in PAIRS:
        health = process_pair(pair_config, state, pending, h4_cache, open_trades, now_utc)
    ...
    if json.dumps(open_trades, sort_keys=True) != trades_before:
        paper_trades.save_open_trades(open_trades)
        print("paper_trades.json updated.")
